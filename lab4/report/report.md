...
### 一、实验目的与背景
本实验聚焦于操作系统内核的进程管理与调度机制，要求深入理解进程的生命周期、内核线程的创建与运行、以及上下文切换的底层实现。通过动手实现关键函数，掌握进程切换的原理和流程，为后续学习多进程、多线程及调度算法打下坚实基础。

操作系统通过进程管理实现多任务并发运行，进程切换是内核调度的核心。上下文切换不仅涉及寄存器、堆栈等硬件状态，还包括地址空间的切换和调度标志的维护。实验以 uCore 内核为基础，要求实现 `proc_run` 等关键函数，体验内核级线程的调度与切换。本实验特别强调了进程统计与调度分工的重要性。
...
{
    if (proc != current)  // 若待运行进程与当前进程相同，无需切换
    {
        unsigned long flags;
        // 1. 关闭中断，禁止中断打断切换过程，保证原子性
        local_intr_save(flags);

        // 2. 保存当前进程指针，供 switch_to 使用
        ...
        // 4. 清除新进程的调度标志
        //    表示该进程已被调度选中，调度器无需再次调度
        //    注意：proc->runs++ 由外层 schedule() 函数执行
        proc->need_resched = 0;

        // 5. 切换页表（地址空间）
        //    更新 SATP 寄存器为新进程的页目录表基址
        //    这样新进程的虚拟地址能正确映射到物理地址
        lsatp(proc->pgdir);
...
**执行流程说明：**
- `switch_to()` 执行后，当前 CPU 已在执行新进程 `proc` 的代码
- 第 7 步的恢复中断代码只在 `proc` 进程再次被调度时执行
- 进程切换具有对称性：A切换到B，B切换到C，C再切换回A时，A在第7步恢复#### 3. 关键流程图解（严谨版本）

```
进程切换流程：proc_run(proc)
   |
   |---> [检查] if (proc != current) ？
   |       |
   |       |---> 是：执行以下步骤
   |       |
   |---> 1. 关闭中断：local_intr_save(flags)
   |       → 保存中断状态，禁止外部中断
   |
   |---> 2. 保存旧进程：struct proc_struct *prev = current
   |       → 备份当前进程指针
   |
   |---> 3. 切换当前进程：current = proc
   |       → 更新内核的当前进程指针
   |
   |---> 4. 清除调度标志：proc->need_resched = 0
   |       → 表示新进程已被调度，不需要再次调度
   |       → 注意：runs++ 由 schedule() 负责，不在此处执行
   |
   |---> 5. 切换页表：lsatp(proc->pgdir)
   |       → 更新 SATP 寄存器
   |       → 激活新进程的虚拟地址空间
   |
   |---> 6. 上下文切换：switch_to(&prev->context, &proc->context)
   |       → 调用汇编函数
   |       → 保存 prev 的寄存器状态到 prev->context
   |       → 恢复 proc 的寄存器状态
   |       → 从 proc 的保存点继续执行（关键！）
   |
   |---> 7. 恢复中断：local_intr_restore(flags)
   |       → 恢复之前保存的中断状态
   |
   |---> [新进程继续执行]
```
...
**执行流程关键点：**
1. `switch_to()` 之后的代码在旧进程被重新调度时才会继续执行
2. 当新进程 `proc` 再次被调度时，它会从 `switch_to()` 之后的代码开始执行
3. 恢复中断 (`local_intr_restore`) 是在该进程被重新调度后才执行

#### 4. 相关宏与函数说明
- `local_intr_save(flags)` / `local_intr_restore(flags)`：用于关、开中断，防止切换过程中被打断。`local_intr_save` 关闭中断并保存状态，`local_intr_restore` 恢复之前的状态（可能是中断打开或关闭）。
- `lsatp(pgdir)`：切换 SATP 寄存器，激活新进程的地址空间。必须在 `switch_to` 之前执行，确保新进程从正确的虚拟地址空间开始执行。
- `switch_to(&prev->context, &proc->context)`：底层汇编实现的上下文切换。执行时保存 `prev` 的所有寄存器到 `prev->context`，然后恢复 `proc->context` 中的寄存器。**关键**：`switch_to` 之后的代码只在该进程再次被调度时才继续执行。
- **重要分工**：`proc->runs++` 由调度函数 `schedule()` 负责（在调用 `proc_run` 前执行），而不是在 `proc_run` 中。这样可以确保统计的一致性和避免重复递增。

#### 5. 代码调试与现象
在调试过程中，通过在 `schedule()` 和 `proc_run()` 等关键位置添加打印语句，观察进程切换和线程创建的过程。系统启动后，首先创建 `idleproc`，随后创建 `initproc`，并输出相关信息，验证切换流程正确。特别需要注意的是：
- 进程的运行计数 `runs` 在 `schedule()` 函数中递增（在调用 `proc_run()` 之前）
- `proc_run()` 中的 `local_intr_restore()` 在进程再次被调度时才执行
- 这样的设计确保了调度统计的准确性和代码的逻辑清晰性
...
- **关键发现**：进程运行次数统计应由 `schedule()` 函数负责，而非 `proc_run`。这体现了系统设计中"职责分离"的原则，使代码更清晰、更易维护。
...
- **职责分离设计**：进程统计（`runs++`）由 `schedule()` 负责，进程切换（`proc_run`）单纯负责上下文和地址空间切换，这样的分工使各函数职责明确，易于维护和扩展。
...
2. **proc_run 的核心实现思路？**  
   答：`proc_run` 执行进程切换的七个关键步骤：（1）关闭中断保证原子性；（2）保存旧进程指针；（3）更新 `current` 指针；（4）清除 `need_resched` 标志（**不执行 `runs++`**，由 `schedule()` 负责）；（5）更新 SATP 寄存器切换页表；（6）调用 `switch_to()` 进行底层上下文切换；（7）进程恢复后恢复中断。**关键认识**：`switch_to()` 之后的代码只在该进程再次被调度时才执行，体现了进程切换的非对称性和中断状态的延迟恢复。

3. **代码编译与运行结果？**  
   答：编译通过，运行后系统能够正确输出进程创建和切换信息，`proc_run` 功能实现符合预期，内核线程能够正常切换和运行。

4. **个人体会与设计思想？**  
   答：本实验深化了对操作系统进程调度机制的理解。特别是通过发现并解决 `runs++` 的重复递增问题，体会到了系统设计中"职责分离"的重要性：`schedule()` 负责选择和统计进程，`proc_run()` 负责执行进程切换，各函数职责明确。这样的设计使代码更易维护、扩展和验证。建议在后续实验中继续深化这一思想，关注进程调度、同步机制、死锁处理等高级内容。
