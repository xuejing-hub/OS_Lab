...
CPU 执行时只是读取指令，并没有发生 store写操作,而 GDB 的 watch 是 监视内存写入，所以这里不会触发。watch *x88208800会在CPU执行写操作(store)到这个地址时触发,但CPU在内核入口只做读取执行(fetch)，没有写入操作。所以即便程序在 0x80200880 执行，GDB 也不会中断。

换句话说：内核实际上已经被加载进了 0x80200000，只是加载过程利用其他手段比如直接映射或者DMA等绕过了 GDB 的内存写检测机制。我们可以通过强制手段“强制验证”内核确实加载到了 0x80200000：可以在 GDB 打断执行后输入：x/16x 0x80200000
