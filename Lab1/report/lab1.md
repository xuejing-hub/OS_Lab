...
在内核早期加载阶段，Bootloader 或 OpenSBI 通常会使用 静态映射（direct mapping） 将内核镜像所在物理地址 0x80200000 映射到固定的虚拟地址，便于 CPU 访问内存。虽然内核写入物理内存的数据仍然有效，但这种映射可能影响 GDB 硬件观察点的触发：

换句话说：内核实际上已经被加载进了 0x80200000，只是加载过程绕过了 GDB 的内存写检测机制。我们可以通过强制手段“强制验证”内核确实加载到了 0x80200000：可以在 GDB 打断执行后输入：x/16x 0x80200000
